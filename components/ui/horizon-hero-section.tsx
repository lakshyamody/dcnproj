...
type MountainMesh = THREE.Mesh<THREE.ShapeGeometry, THREE.MeshBasicMaterial> & {
  userData: { baseZ: number; index: number; targetZ?: number };
};
  ...
  useEffect(() => {
    ...
    const createStarField = () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const stars = new THREE.Points(geometry, material) as StarField;
    ...
    const createMountains = () => {
      ...
      layers.forEach((layer, index) => {
        ...
        const mountain = new THREE.Mesh(geometry, material) as MountainMesh;
    ...
    <div ref={containerRef} className="hero-container cosmos-style">
      ...
      <div className="scroll-sections">
        {[...Array(totalSections)].map((_, i) => {
          const subtitle = SUBTITLES[i + 1] ?? SUBTITLES[0];

          return (
            <section key={i} className="content-section">
              <h1 className="hero-title">{splitTitle(TITLES[i + 1] ?? "DEFAULT")}</h1>
